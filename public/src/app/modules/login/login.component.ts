...
export class LoginComponent {
  ...
  onLogin(): void {
    if (this.loginForm.valid) {
      this.isLoading = true;
      this.http
        .post('https://freeapi.miniprojectideas.com/api/User/Login', this.loginForm.value)
        .subscribe((res: any) => {
  ...
  onRegister(): void {
    if (this.registerFrom.valid) {
      this.isLoading = true;
      this.http
        .post('https://freeapi.miniprojectideas.com/api/User/CreateNewUser', this.registerFrom.value)
        .subscribe((res: any) => {
          this.isLoading = false;
          if (res.result) {
            this.tabIndex = 0;
